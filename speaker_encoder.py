...
def main():
    ...
    # -------------------------
    ckpt_path = "./models/voxceleb/baseline_v2_smproto.model"
    assert os.path.isfile(ckpt_path), "Checkpoint not found"

    state = torch.load(ckpt_path, map_location=device)

    # This checkpoint was saved from SpeakerNet → unwrap if needed
    if "model" in state:
        state = state["model"]
    ...
    for k, v in state.items():
        k = k.replace("__S__.", "").replace("module.", "")
        new_state[k] = v
    ...
    # -------------------------
    # Dummy input (raw waveform)
    # -------------------------
    # eval_frames=400 → 400 * 160 hop ≈ 64000 samples
    batch_size = 2
    num_samples = 64000

    wav = torch.randn(batch_size, num_samples).to(device)
    ...
    print("Input waveform shape :", wav.shape)
    print("Output embedding shape:", emb.shape)
