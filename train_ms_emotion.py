...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
  ...
  for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, y_perturbed, y_perturbed_lengths, speakers, emotions) in enumerate(train_loader):
    ...
    with autocast(enabled=hps.train.fp16_run):
      ...
      (z, z_p, m_p, logs_p, m_q, logs_q),  g, e, zp_pred_spk, zp_pred_emo, spk_pred_from_emo_emb, emo_pred_from_spk_emb, g_logits, e_logits = net_g(x, x_lengths, spec, spec_lengths, speakers, emotions, melspec, perturbed_melspec, y_seg, y_perturbed_seg)
      ...
      with autocast(enabled=False):
        ...
        if(hps.model.grl_type is not None):
          ...
          if(hps.model.grl_type == "classifier"):
            loss_spk_from_emo = F.cross_entropy(spk_pred_from_emo_emb, speakers) * hps.train.c_grl_spkemo
            loss_emo_from_spk = F.cross_entropy(emo_pred_from_spk_emb, emotions) * hps.train.c_grl_spkemo
            loss_zp_spk = F.cross_entropy(zp_pred_spk, speakers) * hps.train.c_grl_zp
